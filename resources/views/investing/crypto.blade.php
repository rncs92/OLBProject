...
<x-app-layout>
        ...
        <div class="flex-auto flex-shrink-0 antialiased bg-gray-50 text-gray-800 ml-64">
            ...
            <div class="grid grid-cols-3 gap-2">
                @foreach($coins as $coin)
                    <div class="bg-white p-4 shadow-md">
                        <div
                            ...
                            <h2 class="text-sm tracking-widest title-font mb-1 font-medium">
                                <strong>{{$coin->getName()}}</strong> ({{ $coin->getSymbol() }})</h2>
                            <h1 class="text-5xl text-gray-900 pb-4 mb-4 border-b border-gray-200 leading-none">{{ "€" . $coin->getPrice() }}</h1>
                    ...
                    </span><strong>Total supply:</strong>{{ $coin->getSupply() }} coins
                            </p>
                            <p class="flex items-center text-gray-600 mb-2">
    <span
        class="w-4 h-4 mr-2 inline-flex items-center justify-center bg-gray-400 text-white rounded-full flex-shrink-0">
    ...
    <span
        ...
        {{ $coin->getPriceChange30d() . "%" }}
                            ...
                            <a href="{{ route('coin', ['symbol' => $coin->getSymbol()]) }}">
                            <button
                            ...
                            <p class="text-xs text-gray-500 mt-3">Think twice before buying and consider risks.</p>
                        </div>
                    </div>
                @endforeach
            </div>
